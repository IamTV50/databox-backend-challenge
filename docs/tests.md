...
No network access, no credentials, no real Databox / GitHub / GitLab calls — the whole suite runs offline.
...
| `tests/gitlab/schema.test.ts` | unit | `rawSchema` accepts a valid project + counts payload, strips unknown project fields, rejects missing counts; `rowSchema` rejects missing metrics |
...
- Fast (~750ms for 48 tests) and deterministic — no flakes from network or API rate limits.
...
- **Real provider contract drift.** GitHub, GitLab, or Databox changing their actual response shape is invisible here. Our `rawSchema` would catch it at runtime, but the test suite wouldn't.
- **OAuth token rotation in the wild.** The test doubles the rotation path but we never exercise a real GitLab refresh; in production the env `GITLAB_REFRESH_TOKEN` is invalidated after the first refresh unless the OAuth app is configured to not rotate, and that interaction isn't covered here.
